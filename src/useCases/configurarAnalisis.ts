...
import { AudioProcessorTrans } from "./audioProcessorUseCaseTrans";
...
import { ParametroAnalisisService } from "../services/index";
...
export async function analizarTranscripcionesBatch(
    servicioSeleccionado: number,
    idsLlamada: string[],
    parametroAnalisis: number
) {
    const parametroAnalisisService = new ParametroAnalisisService();
    const parametrosAnalisisObjeto =
        await parametroAnalisisService.getParametroById(parametroAnalisis);
    console.log(
        "[DEBUG] ID de parametroAnalisis usado en configurarAnalisis:",
        parametrosAnalisisObjeto?.id
    );
    ...
    const audioProcessor = new AudioProcessorTrans(analisisUseCase);
    const resultado = await audioProcessor.procesarTranscripcionesPorIds(
